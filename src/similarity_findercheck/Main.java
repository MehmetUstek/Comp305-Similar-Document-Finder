...
class Main {
   ...
   //we should count number of txt in directory and assign it to n 	
		...
		//Here we stored all of the txt files in here directory, in a txtVertice data structure
 ...
 printer(n, limit,k , array_of_vertice , visited,  layer,groups);	  
	...
	private static void printer(int n,int limit,int source, txtVertice[] Set, int[] visited, int layer ,int [] groups ) { ///  O(n)
		if(visited[source]==0) { 
		...
		//if the result is above the limit than it is putted int arrays which works parallel
		for(int i = 0 ; i< n ; i++) {
